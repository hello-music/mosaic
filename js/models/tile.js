 ...
 */
Tile.prototype.getAvgRBG = function (rowPixelData, canvasWidth, currentTileIndex, unadjustedTileWidth) {
    var x = 0,
        y = 0,
        r = 0,
        g = 0,
        ...
        numOfPixels = this.getNumOfPixels(tileWidth, tileHeight);
    ...
    for (y; y < tileHeight; y += 1) {// loop over each row within the tile
        for (x = 0; x < tileWidth; x += 1) {// loop over each pixel in the tile row
            dataIndex = this.getDataIndex(x, y, canvasWidth, unadjustedTileWidth, currentTileIndex);
 ...
 * @returns {number}
 */
Tile.prototype.getDataIndex = function getDataIndex(x, y, imgWidth, tileWidth, currentTileIndex) {
 ...
 * @returns {number}
 */
Tile.prototype.getNumOfPixels = function getNumOfPixels(width, height) {
    return width * height;
};